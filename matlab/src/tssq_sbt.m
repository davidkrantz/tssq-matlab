...
%                 .acrit, .bcrit: root limit-rule for swtich between modified and standard basis
%                 .use_mod : true => TSSQ (modified/translated basis);
%                            false => SSQ (standard)
%                 .corrR3, .corrR5 : (Fourier engine only) whether to
    ...
    switch lower(opts.basis)
        case 'monomial'
            ...
            [u1,u2,u3,stats] = interpolatory_quadrature_monomial( ...
                xj,yj,zj,sj,wj,f1j,f2j,f3j, ...
                targets(1,:),targets(2,:),targets(3,:), ...
                opts.nquad,edges,opts.rho,opts.acrit,opts.bcrit,opts.upsample, ...
                opts.slender_eps,opts.use_mod);
        case 'fourier'
            ...
            [u1,u2,u3,stats] = interpolatory_quadrature_fourier(...
                ...
                targets(1,:),targets(2,:),targets(3,:), ...
                nquad_global,opts.slender_eps,opts.use_mod,opts.bcrit, ...
                opts.tol,opts.corrR3, opts.corrR5);
    end
end
