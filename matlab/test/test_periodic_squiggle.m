...
% Setup squiggle
[x, y, z, xp, yp, zp, ~, ~, ~] = starfish3D(0.3,3,1);
...
[tjadap, wjadap, npan_adap, ~] = adaptive_panelization(s, nquad_panel, tol);
...
[x2, y2, z2, xp2, yp2, zp2, ~, ~, ~] = starfish3D2(0.3,3,1);
...
%nquad_ref = nquad+2;
nquad_ref = 2*nquad_panel;
tol_ref = 5e-14;
if nquad_panel<16, tol_ref = 1e-2*tol;  end    % low-acc test cases
%Hlim_ref = 1.0;
Hlim_ref = 2.0;
...
[adquad1, adquad2, adquad3, adstats] = adaptive_quadrature(xjadap, yjadap, zjadap, sjadap, wjadap, f1jadap, f2jadap, f3jadap, ...
...
[specquad1,specquad2,specquad3,specquadsh1,specquadsh2,specquadsh3,cancellation_errest,corr_needed,specstats] = interpolatory_quadrature(...
    ...
    x2, y2, z2, s2, nquad_global, slender_eps, tol);
...
if targtype=='s'
  ...
  semilogy(t,cancellation_errest,'.');
  yline(tol,'k',['tol=' num2str(tol)])
  if sum(corr_needed) > 0
    legend('unshifted all','shifted all','shifted corr','adap','cancel errest')
  else
...
if SAVEPLOTS
    exportgraphics(figure(2),['matlab/images/err_squiggle_dist' num2str(dist) '_tol' num2str(tol), '.pdf'],'Resolution',400);
end
...
function [specquad1,specquad2,specquad3,specquadsh1,specquadsh2,specquadsh3,cancellation_errest,corrneeded,stats] = interpolatory_quadrature(...
    xj, yj, zj, sj, tj, wj, f1j, f2j, f3j, X, Y, Z, x, y, z, s, nquad, slender_eps, tol)
